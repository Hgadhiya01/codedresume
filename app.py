...
def experience():
    try:
        if request.method=="POST":
            ...
            if "month" in experience_lower or "year" in experience_lower:
                username = session.get("username", "")
                if username:
                    created_on = updated_on = datetime.now().strftime("%Y/%m/%d %H:%M")
                ...
                flash("Please enter valid experience..........")
                return redirect(url_for('experience', _external=True, _scheme=secure_type))
    ...
    try:
        if request.method=="POST":
            ...
            if "https" in code_link or "http" in code_link or "www" in code_link:
                ...
                flash("Project code URL start with https, http, www..........")
...
def social_media():
    try:
        if request.method=="POST":
            ...
            if "https" in sm_link or "http" in sm_link or "www" in sm_link:
                pass
            else:
                flash("Social-Media URL start with https, http, www..........")
                return redirect(url_for('project', _external=True, _scheme=secure_type))
